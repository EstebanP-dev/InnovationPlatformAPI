...
BEGIN
    ...
    WHILE i < deliverable_ids_length DO
        ...
        SET deliverable_type_id = SUBSTRING_INDEX(deliverable_data, ',', 1);
        SET deliverable_name = SUBSTRING_INDEX(SUBSTRING_INDEX(deliverable_data, ',', 2), ',', -1);
        SET deliverable_url = SUBSTRING_INDEX(SUBSTRING_INDEX(deliverable_data, ',', 3), ',', -1);
        SET deliverable_description = SUBSTRING_INDEX(deliverable_data, ',', 4);

        IF is_valid_uuid(deliverable_type_id) THEN
            INSERT INTO project_deliverables (id, fk_type, fk_project, name, normalized_name, url, description, active, created_at, created_by)
            VALUES (UUID(), deliverable_type_id, project_id, deliverable_name, normalize_text(deliverable_name, TRUE), deliverable_url, deliverable_description, 1, NOW(), USER());
        ELSE
